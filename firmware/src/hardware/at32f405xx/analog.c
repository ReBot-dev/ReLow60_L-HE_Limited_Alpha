...
#if ADC_NUM_MUX_INPUTS > 0
void analog_settling_capture(uint8_t key, uint16_t *buf, uint16_t num_samples) {
  uint8_t target_mux_ch = 0;
  uint8_t target_input = 0;
  bool found = false;

  for (uint8_t ch = 0; ch < (1 << ADC_NUM_MUX_SELECT_PINS) && !found; ch++) {
    for (uint8_t inp = 0; inp < ADC_NUM_MUX_INPUTS; inp++) {
      if (mux_input_matrix[ch][inp] == (uint16_t)(key + 1)) {
        target_mux_ch = ch;
        target_input = inp;
        found = true;
        break;
  ...
  // Reconfigure ADC: single channel, single conversion, no DMA
  ...
  adc_ordinary_channel_set(
      ADC1, (adc_channel_select_type)mux_input_channels[target_input], 1,
      ADC_NUM_SAMPLE_CYCLES);

  // Set MUX to a different channel and wait for full settling
  uint8_t other_ch =
      (target_mux_ch + 4) & ((1 << ADC_NUM_MUX_SELECT_PINS) - 1);
  for (uint32_t i = 0; i < ADC_NUM_MUX_SELECT_PINS; i++)
    gpio_bits_write(mux_select_ports[i], mux_select_pins[i],
                    (confirm_state)((other_ch >> i) & 1));
  for (volatile uint32_t i = 0; i < F_CPU / 1000; i++)
    ;

  // Switch to target channel (no delay)
  for (uint32_t i = 0; i < ADC_NUM_MUX_SELECT_PINS; i++)
    gpio_bits_write(mux_select_ports[i], mux_select_pins[i],
                    (confirm_state)((target_mux_ch >> i) & 1));
  ...
  for (uint16_t s = 0; s < num_samples; s++) {
    ...
    while (adc_flag_get(ADC1, ADC_CCE_FLAG) == RESET)
      ;
    buf[s] = adc_ordinary_conversion_data_get(ADC1);
