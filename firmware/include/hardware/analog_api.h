...
#if ADC_NUM_MUX_INPUTS > 0
/**
 * @brief Capture ADC samples immediately after MUX switch for settling analysis
 *
 * Temporarily stops normal scanning, switches MUX from a different channel to
 * the target key's channel, and rapidly samples ADC without any settling delay.
 *
 * @param key Key index
 * @param buf Buffer to store samples
